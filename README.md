...
- **アニメ GIF は image では動かない**（LINE 仕様で静止画化される）。動かすなら **`send_video`** を使う—GIF/動画を ffmpeg で mp4（H.264 baseline + yuv420p + 16の倍数解像度 + 無音音声トラック + faststart）に変換し、video メッセージとして送ると**トークで自動再生**される。`ffmpeg` が必要。動画も「push成功＝受理」で、取得まで配信サーバーの常駐が要るのは画像と同じ。
