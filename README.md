...
`setup.sh` が `deploy/com.line-bridge.static.plist` を実値で生成して `~/Library/LaunchAgents/` に配置・load 済み。確認・操作:
```bash
...
> 📚 **ここから下はリファレンス**です（セットアップ後、必要なときに参照）: 提供ツール / 構成ファイル / 送信ログ / クォータ / 制約 / トラブル / 対応OS。
...
```
sLINE/
├── server.py            # MCP 本体（4ツール + 静的配信サーバー）。uv の実行エントリ＝ルート固定
├── get_user_id.py       # userId 取得ヘルパー（セットアップ時に一度だけ使う）
├── smoke_test.py        # 認証不要の自己テスト（CI が全OSで実行）
├── setup.sh / setup.ps1 # セットアップ自動化（macOS・Linux / Windows）
├── pyproject.toml       # 依存定義（uv 管理）
├── .env.example         # 設定テンプレート（→ .env にコピー）
├── AGENTS.md            # AI エージェント向けの道標
├── deploy/
...
> 実行時に生成（`.gitignore` 済み）: `public/`（配信メディア・1時間で自動削除） / `logs/`（送信・遅延ログ） / `.env`（秘密情報）

| ファイル | 役割 |
|---|---|
| `server.py` | MCP 本体。`send_text` / `send_image` / `send_images` / `send_video` / `send_file` / `send_location` / `send_buttons` / `send_stats` ツール + メディア配信用の静的サーバー内蔵 |
...
| `deploy/com.line-bridge.static.plist` | 配信サーバーを常駐させる launchd テンプレート（`setup.sh` が実値を埋めて配置） |
| `pyproject.toml` | 依存定義（uv 管理） |
...
- **`send_location(latitude, longitude, title="", address="")`** — 地図ピン（LINE ネイティブの location）を送る。メディア配信なし（push のみ）。
- **`send_buttons(text, buttons, title="")`** — 本文＋URL ボタン（最大4・https 必須）のカードを送る。「処理完了→[PR を開く][成果物を見る]」のような**アクション付き通知**に。push のみ。
...
- **無料枠は 200通/月**（push 1回 = 1通、吹き出しを最大5つ詰めても1通）。**上限到達で送信不可になるだけで課金は発生しない**（従量課金はスタンダードプランのみ）。本ツールは安全上限（既定180/月）で**送信前に自動で止める** → 詳細は上記「月間送信クォータ」セクション。
...
- **動画はスマホが「再生時」に直接 mp4 を取得する**（実測: iPhone の `AppleCoreMedia` が Range 取得）。画像は LINE がキャッシュ配信するが、**動画はキャッシュされず端末が都度取りに来る**ため、**再生する瞬間に Mac が起きていて配信が生きている必要がある**。送信後は `caffeinate` でスリープ抑制を試みるが、**バッテリー駆動で「ふたを閉じる」スリープは防げない** → 送ったら**すぐ開く**か、**電源接続＋ふた開け**（または常時起動サーバーで配信）で確実に。怠ると「受信できたのに再生できない（音だけ/真っ黒/くるくる）」になる。
