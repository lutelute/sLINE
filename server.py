...
# しうるため、スレッドロック（_usage_lock）に加えて fcntl.flock でプロセス間も直列化する。
# logs/ はローカルディスク前提（flock は NFS/SMB では非保証）。
...
def _with_usage_locked(fn):
    ...
    with _usage_lock:                       # 同一プロセス内スレッドの直列化
        with open(USAGE_FILE, "r+", encoding="utf-8") as f:
            fcntl.flock(f.fileno(), fcntl.LOCK_EX)   # 別プロセスとの直列化
            try:
                raw = f.read()
                try:
...
def keep_awake(seconds: int = 120) -> None:
    """画像送信後しばらく Mac をスリープさせない（best-effort, macOS のみ）。

    注意: caffeinate はバッテリー駆動時の「ふた閉じ」スリープは防げない。
    確実にしたい場合は常時起動のサーバーで配信すること。
    """
    try:
        subprocess.Popen(
