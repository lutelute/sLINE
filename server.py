...
# 任意ファイル送信（send_file）で配信を許可する拡張子。LINE はボットからのファイル添付に
# 非対応なので、ファイルは公開ディレクトリに置き「タップで開けるURL」をテキストで送る。
# ブラウザでスクリプト実行され得る型（html/htm/svg/xml/xhtml/js）は全インターネット公開の
# ため意図的に除外する。上限は env LINE_FILE_MAX_BYTES で変更可（既定 100 MB）。
FILE_EXT_ALLOW = (
    "pdf", "txt", "md", "csv", "tsv", "json", "log", "rtf",
    "zip", "gz", "tar", "tgz", "7z",
    "doc", "docx", "xls", "xlsx", "ppt", "pptx",
)
FILE_MAX_BYTES = _int_env("LINE_FILE_MAX_BYTES", 100 * 1000 * 1000)

# token は secrets.token_hex(16) = 32 桁の16進。配信サーバーはこのパターンのみ許可する。
#   画像/動画 : <token>.jpg|png|mp4
#   preview   : <token>_preview.jpg|png
#   任意ファイル: <token>-<安全名>.<許可拡張子>（'/' や '..' を含まない固定形）
_FILE_EXT_ALT = "|".join(FILE_EXT_ALLOW)
TOKEN_FILE_RE = re.compile(
    rf"^/[0-9a-f]{{32}}"
    rf"(\.(jpg|png|mp4)"
    rf"|_preview\.(jpg|png)"
    rf"|-[A-Za-z0-9._-]{{1,80}}\.({_FILE_EXT_ALT})"
    rf")$"
)
...
def compute_stats(limit: int = 20) -> list[dict]:
    ...
    for s in sends[-limit:]:
        ...
        epoch = s.get("epoch")
        if s.get("ok") and isinstance(epoch, (int, float)) and epoch > 0:
            # 画像/動画は preview/original、ファイルは配信名(name)を取得記録と突合する
            if s.get("type") in ("image", "video"):
                keys = (s.get("preview"), s.get("original"))
            elif s.get("type") == "file":
                keys = (s.get("name"),)
            else:
                keys = ()
            cands = [first_fetch[n] for n in keys if n and n in first_fetch]
            if cands:
                d = min(cands) - epoch
...
class _QuietHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def _serve(self, path, head: bool) -> None:
        ...
        try:
            ...
            self.send_error(404)
            return
        ctype = self.guess_type(str(path)) or "application/octet-stream"
...
def send_file(path: str, caption: str = "") -> str:
    ...
    LINE はボット(Messaging API)からのファイル添付に非対応なので、ファイルをこのマシンの
    公開ディレクトリに置き、Tailscale Funnel 経由の公開 HTTPS URL をテキストで送る。受信側は
    URL をタップしてブラウザで開く / ダウンロードする。画像は send_image、動画は send_video の
    方がチャット内に表示されるため、それらの拡張子は案内して中止する。

    対応拡張子: pdf, txt, md, csv, tsv, json, log, rtf, zip, gz, tar, tgz, 7z,
    doc, docx, xls, xlsx, ppt, pptx（上限は既定 100MB / env LINE_FILE_MAX_BYTES で変更可）。

    Args:
        path: 送りたいファイルの絶対パス。
        caption: 添えるテキスト（任意。URL と同じメッセージに同梱）。
    Returns:
        送信結果の短い説明。
    """
    global _last_image_send
    ...
    if ext in ("jpg", "jpeg", "png", "gif"):
        return f"ℹ️ 画像は send_image で送るとチャット内に表示されます（{src.name}）。"
    if ext in ("mp4", "mov", "webm", "avi", "mkv"):
        return f"ℹ️ 動画は send_video で送るとチャット内で再生できます（{src.name}）。"
    if ext not in FILE_EXT_ALLOW:
        return (
            f"エラー: 拡張子 .{ext or '(なし)'} は未対応です。対応: " + ", ".join(FILE_EXT_ALLOW)
        )

    size = src.stat().st_size
    if size > FILE_MAX_BYTES:
        return (
            f"エラー: ファイルが大きすぎます（{size // (1000 * 1000)}MB）。"
            f"上限 {FILE_MAX_BYTES // (1000 * 1000)}MB（env LINE_FILE_MAX_BYTES で変更可）。"
        )
    ...
    base = public_base_url()
    token = secrets.token_hex(16)
    served = public_dir() / f"{token}-{_safe_stem(src.stem)}.{ext}"
    try:
        shutil.copy2(src, served)
    except OSError as e:
        return f"エラー: ファイルの配置に失敗しました: {e}"

    url = f"{base}/{served.name}"
    if len(url) > URL_MAX_CHARS:
        _safe_unlink(served)
        return "エラー: 生成された URL が 2000 文字を超えました。ファイル名を短くしてください。"
    # 送る前に「実際に配信できているか」をローカルで確認（届かない送信を防ぐ）
    if not verify_served(served.name):
        _safe_unlink(served)
        return (
            f"エラー: ポート {static_port()} でファイルを配信できていません"
            "（静的サーバー未起動 / ポート占有の可能性）。ファイルは送信しませんでした。"
        )

    body = ""
    if caption and caption.strip():
        body += caption.strip() + "\n\n"
    body += f"📎 {src.name}（{size // 1024}KB）\n{url}"
    body = body[:TEXT_MAX_CHARS]

    # 送信直前に 1 通を予約（1 push = 1通）
    try:
        granted, used, limit = reserve_quota(1)
    except OSError as e:
        _safe_unlink(served)
    ...
    t0 = time.time()
    try:
        line_push([{"type": "text", "text": body}])
    except Exception as e:
        release_quota(1)
        _safe_unlink(served)
        ep = time.time()
        log_event({"event": "send", "ts": _iso(ep), "epoch": ep, "type": "file",
                   "ok": False, "token": token, "err": str(e)[:300]})
        return f"エラー: LINE への送信に失敗しました: {e}"

    ack = time.time()
    push_ms = int((ack - t0) * 1000)
    _last_image_send = ack  # ファイルも LINE/受信側が後から取りに来るので終了猶予の対象にする
    keep_awake()
    log_event({"event": "send", "ts": _iso(ack), "epoch": ack, "type": "file", "ok": True,
               "token": token, "bytes": size, "push_ms": push_ms,
               "name": served.name, "orig": src.name, "used": used, "limit": limit})
    return (
        f"LINE にファイルのリンクを送信しました: {url} "
